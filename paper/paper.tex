...
al.}\shortcite{sheridan:ijppr}.  For example Mitchell \emph{et
al.}\shortcite{mitchell:graphss} looked for common sub-structures by
...
Jones\shortcite{kleywegt:dejavu} described a protein as a set of SSEs
...
labelled A--D
% coloured blue, green, orange, red
...
%coloured from blue at the N-terminus
%(residue 433) to red at the C-terminus (residue 483). 
Note the
...
\begin{figure}
\centerline{\psfig{file=pics/cutoffs.eps,width=2.5in}}
...
%     However, what we are really interested in is being able to hit just
%     one protein of the correct fold. A more detailed comparison can then
%     be performed against all examples from a representative set having the
%     same fold.
%     
%     % This was done using _n_a_l_L_ dataset as follows:
%     % Filter out those with <4 SSEs
%     %    filterranks.perl ranks_e3h3_n_a_l_L_nolimit.out >ranks_e3h3_n_a_l_L_nolimit.filtered
%     % Find the best score for each true match
%     %    findcutoff.perl ranks_e3h3_n_a_l_L_nolimit.filtered >cutoff_e3h3_n_a_l_L_nolimit_filtered.out
%     % Sort this file
%     %    sort -n +1 cutoff_e3h3_n_a_l_L_nolimit_filtered.out >cutoff_e3h3_n_a_l_L_nolimit_filtered.sor
%     % See how may examples we have
%     %    wc cutoff_e3h3_n_a_l_L_nolimit_filtered.sor
%     % (= 2575) Therefore 1% = 26, 5% = 129
%     % Find the cutoff for 1% and 5%
%     %    scoreforfirstN.pl -count=26 cutoff_e3h3_n_a_l_L_nolimit_filtered.sor
%     %    scoreforfirstN.pl -count=129 cutoff_e3h3_n_a_l_L_nolimit_filtered.sor
%     % (26 => 33.33 ; 129 => 46.25)
%     % Calculate the error rate at the cutoffs
%     %    errorAtCutoff.perl -cutoff=33.33 ranks_e3h3_n_a_l_L_nolimit.filtered
%     %    errorAtCutoff.perl -cutoff=46.25 ranks_e3h3_n_a_l_L_nolimit.filtered
%     % (1% (33.33) => 85.08% ; 5% (46.25) => 62.48% )
%     
%     As before, each protein fold containing more than one near-identical
%     sequence family was examined. For each of these families, the maximum
%     topscan score for a correct hit against another protein domain with
%     the same fold was recorded. Trivial matches in which either structure
%     contains fewer than 4 SSEs were ignored. A barchart of frequency of
%     scores (in bins of five percentage units) against score is shown in
%     \figref{~\ref{fig:cutoffs}}. From this distribution, scores of 33.33\%
%     and 46.25\% can be determined as cutoffs at which 1\% and 5\%
%     respectively of correct hits would be lost. i.e. A coverage of 99\%
%     would be achieved by selecting only hits with a score better than
%     33.33\%.
%     
%     The error rate for each cutoff, $c$, was then calculated as:
%     $$E = \frac{N_{f,c}}{N_{f,c} + N_{t,c}}$$ 
%     where $N_{f,c}$ is the number of false positive matches scoring $\ge
%     c$ and $N_{t,c}$ is the number of true positive matches scoring $\ge
%     c$ number of false positive matches scoring $\ge$ these cutoffs was
%     then calculated as a percentage of the total number of matches.
%     Error rates of 85.08\% at 99\% coverage and 63.48\% at 5\%
%     coverage were achived.
%     
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusions}
TOPSCAN has proved a useful addition to the available methods for
comparing protein structures.  TOPSCAN was initially developed as a
simple method to reduce the search space for the CATH-server. In that
implementation, the secondary topology strings contain only the
directional information and it is used as a rapid method of ranking
structures for further more detailed comparison using SSAP.

Analysis of the CATH data using TOPSCAN has shown some interesting
anomalies as well as a few minor errors in the CATH data which have
been fed back to the CATH maintainers. Thus TOPSCAN also appears to
have a role in rapid validation of structural classification.

Our analysis of factors affecting the performance of TOPSCAN has
suggested that treating \htt-helix as $\alpha$-helix in secondary
structure prediction is a bad strategy especially when the results are
to be used for fold recognition.

In common other methods which reduce a three-dimensional structure to
a set of secondary structure elements with or without associated
vector information, TOPSCAN is most likely to fail when a structure is
poorly defined and DSSP or STRIDE are unable to make reliable
secondary structure assignments. The atomic level full SSAP is less
prone to these problems, but pays a huge penalty in the time required
for comparison.
...
classifications are used (e.g.\ exposed vs. buried). In the case of
...
feed back into fold recognition and the algorithm described here, when
...
A TOPSCAN server has been made available at
\verb1http://www.bioinf.org.uk/topscan/1 and at \verb1http://www.rubic.rdg.ac.uk/topscan/1
